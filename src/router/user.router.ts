import { body, formData, path, request, summary, tags } from 'koa-swagger-decorator';
...
export class UserRouter {
    ...
    @formData({
        file: {type: 'file', required: false, description: 'avatar'}
    })
    @body({
        avatar_id: {type: 'number', required: false, example: 545},
    ...
    static async updateUser(ctx: any): Promise<void> {
        try {
            ...
            await userService.updateUser(id, {avatarId: file.id});
            ctx.response.body = await userService.getUser(id);
        } catch (e) {
            throw new HttpServerError(e);
        }
